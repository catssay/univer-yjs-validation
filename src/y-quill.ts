 ...
 * @param {any} delta
 */
export function normQuillDelta(delta) {
  if (delta.length > 0) {
 ...
 * @param {Y.Text} type
 */
function updateCursor(quillCursors, aw, clientId, doc, type) {
  try {
 ...
 */
function typeDeltaToQuillDelta(delta, binding) {
  return delta.map((op) => {
    ...
    this._typeObserver = (_events, tr) => {
      if (tr.origin !== this) {
        ...
        const equals = appliedDelta.ops.length === delta.ops.length && appliedDelta.ops.every((op, i) => {
          ...
          if (op.insert != null)
            return op.insert === otherOp.insert || (typeof op.insert === 'object' && typeof op.insert === typeof otherOp.insert)
          if (op.retain != null)
            return op.retain === otherOp.retain || (typeof op.retain === 'object' && typeof op.retain === typeof otherOp.retain)
